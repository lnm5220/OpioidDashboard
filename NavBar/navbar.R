...
HCUPKFFdata <- fread("./data.csv", quote="")
  ...
  tabPanel("Single-State Analysis",
           fluidRow(
             ...
             box(
               title = "Controls",
               width = 4,
               selectInput("dropdown_state_input", "Select State", c("Select a State" = "", Policy$State)),
               checkboxGroupButtons("single_state_checkbox", "Select Metrics to compare on Line Graph",
                                    choices = list("Inpatient Opioid Related Hospitalizations" = "IP",
                                                   "Emergency Department Opioid Related Hospitalizations" = "ED",
                                                   "Natural Semisynthetic Overdose Deaths (e.g. Oxycodone)"="OverdoseDeathsNaturalSemisynthetic",
                                                   "Methadone Overdose Deaths"="OverdoseDeathsMethadone",
...
server <- function(input, output) {
  ...
  observeEvent(input$single_state_checkbox, {
    ...
    filter_single_data <- reactive({
      #get the year & measure from main data
      single_data <- HCUPKFFdata %>% filter(HCUPKFFdata$name == input$dropdown_state_input) %>% select(input$single_state_checkbox,"name","Year")
      return(single_data)
    })
    
    single.line.graph <- reactive({
      single_data <- filter_single_data()
      plot_ly(single_data, x = ~Year,y = single_data[[input$single_state_checkbox]],color = input$single_state_checkbox , type = 'scatter', mode = 'lines', connectgaps = FALSE) %>% 
