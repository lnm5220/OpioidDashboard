...
###Load in KFF/HCUP
HCUPKFFdata <- fread("C:/Users/Andrew Briglia/Documents/R/OpioidDashboard/dt.KFF_HCUP.csv", quote="")

#filter data to only get annual results and get rid of extra territories
#CONDENSE THIS TO ONE FILTER STATEMENT
HCUP_KFF_annual <- dt.KFF_HCUP %>%
...
ui <- dashboardPage(
  dashboardHeader(title = "Opioid Policy & Data Dashboard"),
  ##Sidebar Content
  sidebar <- dashboardSidebar(
    sidebarMenu(
      menuItem("Multi State Analysis", tabName = "tab1", icon = icon("dashboard")),
      menuItem("Single-State Analysis", tabName = "tab2", icon = icon("th"))
    )
  ),
  ##Body Content
  dashboardBody(
