...
dropdown_options <- c("IP","ED")
#usa$IP <- HCUP_annual$IP
year_list <- c(unique(HCUP_annual$Year))
...
server <- function( input, output, session ){
    ...
    foundational.map <- reactive({
        ...
        filter_data <- reactive({
            ...
            data <-  HCUP_annual %>% filter(HCUP_annual$Year == input$dropdown_year)
        ...
        leaflet() %>%
            ...
            addLegend(pal = pal, values = data[[input$dropdown]], opacity = 1, title = paste0(input$dropdown_year," ",input$dropdown),
    ...
    observeEvent(
        input$myMap_shape_click, {
            ...
            filter_state_data <- reactive({
                #filter data based on input year and then select measure
                data <-  HCUP_annual %>% filter(State %in% click.list$ids)
            })
